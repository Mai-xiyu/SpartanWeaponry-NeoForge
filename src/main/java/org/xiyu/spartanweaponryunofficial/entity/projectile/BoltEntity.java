...
public class BoltEntity extends AbstractArrow implements IEntityWithComplexSpawn {
    ...
    protected static final EntityDataAccessor<ItemStack> DATA_BOLT =
            SynchedEntityData.defineId(BoltEntity.class, EntityDataSerializers.ITEM_STACK);
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        DamageSource source;
        if (shooter == null) source = ModDamageTypes.armorPiercingProjectile(this, this);
        else {
            source = ModDamageTypes.armorPiercingProjectile(this, shooter);
            if (shooter instanceof LivingEntity) ((LivingEntity) shooter).setLastHurtMob(entity);
        }

        boolean isEnderman = entity.getType() == EntityType.ENDERMAN;
        int fireTimer = entity.getRemainingFireTicks();
        if (this.isOnFire() && !isEnderman) entity.igniteForSeconds(5.0F);
        ...
        if (entity.hurt(source, (float) damage)) {
            ...
            if (entity instanceof LivingEntity livingentity) {
                ...
                if (!level.isClientSide
                        && shooter instanceof ServerPlayer serverplayerentity
                        && !entity.isAlive()
                        && this.shotFromCrossbow())
                    CriteriaTriggers.KILLED_BY_CROSSBOW.trigger(
                            serverplayerentity, List.of(entity));
            }
    ...
    public ResourceLocation getTexture() {
        ...
        return ResourceLocation.fromNamespaceAndPath(
                ModSpartanWeaponry.ID, "textures/entity/projectiles/" + boltRegName + ".png");
    }
}
