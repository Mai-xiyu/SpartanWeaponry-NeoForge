...
/*		ImmutableList.of(Items.WOODEN_SWORD, Items.STONE_SWORD, Items.IRON_SWORD, Items.GOLDEN_SWORD, Items.DIAMOND_SWORD, Items.NETHERITE_SWORD).
    ...
    protected ResourceLocation createTippedArrowModel(Item item) {
        ...
        return this.withExistingParent(itemPath, this.mcLoc("item/generated")).texture("layer0", "item/" + itemPath + "_base").texture("layer1", "item/tipped_arrow_head").getLocation();
    }

    protected ResourceLocation createTippedBoltModel(Item item) {
        String itemPath = BuiltInRegistries.ITEM.getKey(item).getPath();
        return this.withExistingParent(itemPath, this.mcLoc("item/generated")).texture("layer0", "item/" + itemPath + "_base").texture("layer1", "item/tipped_bolt_head").getLocation();
    }
    ...
    protected ResourceLocation createQuiverModels(Item item, int variantCount) {
        ...
        for (int i = 0; i < variantCount; i++) {
            ...
            variants.add(this.withExistingParent(modelVariant, this.mcLoc("item/generated")).texture("layer0", "item/" + modelVariant).getLocation());
        }
        ItemModelBuilder modelBuilder = this.withExistingParent(itemPath, this.mcLoc("item/generated")).texture("layer0", "item/" + itemPath + "_base");
        for (int j = 0; j < variants.size(); j++) {
            modelBuilder.override().predicate(ModelOverrides.ARROW, j + 1).model(new ExistingModelFile(variants.get(j), this.existingFileHelper)).end();
        }
    ...
    protected ResourceLocation createWeaponOilModel(Item item) {
        String itemPath = BuiltInRegistries.ITEM.getKey(item).getPath();
        return this.withExistingParent(itemPath, this.mcLoc("item/generated")).texture("layer0", "item/" + itemPath + "_bottle").texture("layer1", "item/" + itemPath + "_bottle_overlay").getLocation();
    }
