...
public class VillagerTrades {
    public static class BaseTrade implements ItemListing {
        ...
        public MerchantOffer getOffer(@NotNull Entity trader, @NotNull RandomSource rand) {
            ...
            Optional<ItemCost> cost2 = this.wantedStack2.isEmpty() ? Optional.empty() : Optional.of(new ItemCost(this.wantedStack2.getItem(), this.wantedStack2.getCount()));
            return new MerchantOffer(cost1, cost2, this.offeredStack, this.maxUses, this.xpGiven, this.priceMultiplier);
        }
    }
    ...
    public static class BuyEnchantedItemWithEmeraldsTrade extends BuyItemWithEmeraldsTrade {
        ...
        public MerchantOffer getOffer(@NotNull Entity trader, @NotNull RandomSource rand) {
            ...
            Stream<Holder<Enchantment>> enchantments = trader.level().registryAccess().registryOrThrow(Registries.ENCHANTMENT).holders().map(h -> h);
            ItemStack enchantedStack = EnchantmentHelper.enchantItem(rand, new ItemStack(this.offeredStack.getItem()), level, enchantments);
            ItemCost cost = new ItemCost(this.wantedStack.getItem(), this.wantedStack.getCount());
            return new MerchantOffer(cost, Optional.empty(), enchantedStack, this.maxUses, this.xpGiven, this.priceMultiplier);
        }
    }
    ...
    public static class RandomisedBuyItemWithEmeraldsTrade implements ItemListing {
        ...
        @Override
        public MerchantOffer getOffer(@NotNull Entity trader, @NotNull RandomSource rand) {
            RandomisedTradeItem offeredItem = this.offeredItems.get(this.offeredItems.size() == 1 ? 0 : rand.nextInt(this.offeredItems.size() - 1));
            ItemCost cost = new ItemCost(Items.EMERALD, offeredItem.getEmeraldCost());
            return new MerchantOffer(cost, Optional.empty(), offeredItem.getItemStack(), this.maxUses, this.xpGiven, this.priceMultiplier);
        }
    ...
    public static class RandomisedBuyEnchantedItemWithEmeraldsTrade extends RandomisedBuyItemWithEmeraldsTrade {
        ...
        @Override
        public MerchantOffer getOffer(@NotNull Entity trader, @NotNull RandomSource rand) {
            RandomisedTradeItem offeredItem = this.offeredItems.get(rand.nextInt(this.offeredItems.size() - 1));
            int level = 5 + rand.nextInt(15);
            Stream<Holder<Enchantment>> enchantments = trader.level().registryAccess().registryOrThrow(Registries.ENCHANTMENT).holders().map(h -> h);
