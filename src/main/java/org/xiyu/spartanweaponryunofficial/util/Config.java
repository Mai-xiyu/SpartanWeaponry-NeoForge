...
@Mod.EventBusSubscriber(modid = ModSpartanWeaponry.ID, bus = Mod.EventBusSubscriber.Bus.MOD)
public class Config {
    public static final Config INSTANCE;
    public static final ForgeConfigSpec CONFIG_SPEC;

    protected final Predicate<Object> IS_VALID_RESOURCE_LOCATION = (entry) -> ResourceLocation.tryParse(entry.toString()) != null;
    ...
    private Config(ForgeConfigSpec.Builder builder) {
        ...
        this.explosionStrengthDynamite = builder.comment("Explosion strength for Dynamite|炸药的爆炸强度")
                ...
                .defineInRange("explosion_strength_dynamite", (double)Defaults.ExplosionStrengthDynamite, 0.1d, 10.0d);
        builder.pop();
...
//		ModItems.SCYTHES.updateSettingsFromConfig(INSTANCE.scythes.baseDamage.get().floatValue(), INSTANCE.scythes.damageMultipler.get().floatValue(), INSTANCE.scythes.speed.get().doubleValue());
        ...
        // Update Oils - Use Forge's registry access method
        OilEffects.REGISTRY.getEntries().forEach(entry -> {
            OilEffect effect = entry.get();
