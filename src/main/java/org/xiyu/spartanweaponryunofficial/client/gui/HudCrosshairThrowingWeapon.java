    ...
    public static void render(
        ...
        // otherwise abort the rendering
        {
            ...
            if (player.isUsingItem()) {
                int maxChargeTicks =
                        Math.max(1, throwingWeapon.getMaxChargeTicks(equippedStack, mc.level));
                float percentage =
                        Mth.clamp(
                                (player.getTicksUsingItem() + partialTicks) / maxChargeTicks,
                                0.0f,
                                1.0f);
            ...
            if (ItemStackDataHelper.getTag(equippedStack)
                    .contains(ThrowingWeaponItem.NBT_AMMO_USED)) {
                ...
                //                mc.font.drawInBatch(text, textX - (mc.font.width(text) / 2),
                // textY,
                // 0xFFFFFFFF,
                //                        true, poseStack.last().pose(), buffer,
                ...
                guiGraphics.drawString(
                        mc.font, text, textX - (mc.font.width(text) / 2), textY, 0xFFFFFFFF);
                guiPose.popPose();
