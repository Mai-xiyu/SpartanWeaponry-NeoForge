...
public class HudCrosshair {
    public static final ResourceLocation CROSSHAIR_TEXTURES =
            ResourceLocation.fromNamespaceAndPath(
                    ModSpartanWeaponry.ID, "textures/gui/crosshairs.png");
    public static final ResourceLocation ICONS_LOCATION =
            ResourceLocation.parse("textures/gui/icons.png");
    ...
    public static void render(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        if (equipStack.isEmpty()) {
            if (isVanillaCrosshairDisabled) isVanillaCrosshairDisabled = false;
            ...
            if (crosshairItem.getCrosshairHudElement() != null) {
                ...
                if (!isVanillaCrosshairDisabled) isVanillaCrosshairDisabled = true;

                if ((options.getCameraType().isFirstPerson()
                                || ModList.get().isLoaded("leawind_third_person")
                                || (!ClientConfig.INSTANCE.disableShoulderSurfingIntegration.get()
                                        && ModList.get().isLoaded("shouldersurfing")
                                        && ShoulderSurfingCompat.isShoulderSurfing()))
                        && (mc.gameMode.getPlayerMode() != GameType.SPECTATOR
                                || canRenderCrosshairForSpectator(mc))) {
                    // Do the debug rendering for crosshairs even with the custom crosshairs enabled
                    if (mc.getDebugOverlay().showDebugScreen()
                            && !options.hideGui
                            && !player.isReducedDebugInfo()
                            && !options.reducedDebugInfo().get()) {
                        Camera camera = mc.gameRenderer.getMainCamera();
                        Matrix4fStack modelViewStack = RenderSystem.getModelViewStack();
                        modelViewStack.pushMatrix();
                        modelViewStack.translate(
                                (float) (screenWidth / 2), (float) (screenHeight / 2), 0.0f);
                        ...
                        crosshairItem
                                .getCrosshairHudElement()
                                .render(guiGraphics, deltaTracker, equipStack);
                }
            }
        }
    }
