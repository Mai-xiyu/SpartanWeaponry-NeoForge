...
public class HudQuiverAmmo {
    ...
    public static void render(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        ListTag list = ItemStackDataHelper.getTag(quiverStack).getCompound(QuiverBaseItem.NBT_AMMO).getList("Items", Tag.TAG_COMPOUND);

        for (int i = 0; i < list.size(); i++) {
            ItemStack ammoStack = ItemStack.parseOptional(mc.level.registryAccess(), list.getCompound(i));
