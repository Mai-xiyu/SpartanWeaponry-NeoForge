...
public abstract class QuiverBaseMenu extends AbstractContainerMenu 
{
	...
	protected QuiverBaseMenu(MenuType<?> type, int id, Inventory inventory, ItemStack quiverStackIn, Predicate<ItemStack> slotFilterIn, ResourceLocation emptySlotTextureIn)
	{
		...
		IQuiverItemHandler handlerTmp = quiverStack.getCapability(ModCapabilities.QUIVER_ITEM_CAPABILITY);
		if(handlerTmp == null && quiverStack.getItem() instanceof QuiverBaseItem quiverItem)
			handlerTmp = new QuiverItemStackHandler(quiverStack, quiverItem.getAmmoSlots());
		handler = handlerTmp;
	...
	{
		if(slot >= 0 && getSlot(slot) != null && (ItemStack.isSameItemSameComponents(getSlot(slot).getItem(), quiverStack)))
			return;
