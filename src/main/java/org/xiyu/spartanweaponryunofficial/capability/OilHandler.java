...
public class OilHandler implements IOilHandler {
    ...
    public OilHandler(ItemStack stackIn) {
        ...
        if (ItemStackDataHelper.getTag(this.stack).contains(NBT_OIL))
            this.deserializeNBT(
                    getRegistryAccess(),
                    ItemStackDataHelper.getTag(this.stack).getCompound(NBT_OIL));
    }
    ...
    public void setEffect(OilEffect effectIn, ItemStack oilStackIn) {
        ...
        this.potion =
                effectIn == OilEffects.POTION.get()
                        ? Optional.ofNullable(OilHelper.getPotionFromStack(oilStackIn))
                        : Optional.empty();
        ItemStackDataHelper.updateTag(
                this.stack, tag -> tag.put(NBT_OIL, this.serializeNBT(getRegistryAccess())));
    }
                ...
                ((Player) userIn)
                        .displayClientMessage(
                                Component.translatable(
                                        "message." + ModSpartanWeaponry.ID + ".oil_depleted",
                                        oilStack.getHoverName(),
                                        userWeaponIn.getHoverName()),
                                true);
            this.clearEffect();
        }
        return resultDamage;
    ...
    public void deserializeNBT(HolderLookup.@NotNull Provider provider, CompoundTag nbt) {
        ...
        if (registry != null) {
            this.effect =
                    !oilEffectReg.isEmpty()
                            ? Optional.ofNullable(
                                    registry.get(ResourceLocation.parse(oilEffectReg)))
                            : Optional.empty();
                        ...
                        !potionReg.isEmpty()
                                ? Optional.ofNullable(
                                        BuiltInRegistries.POTION.get(
                                                ResourceLocation.parse(potionReg)))
                                : Optional.empty();
            }
        }
