...
/**
 * Oil Effects allow compatible weapons to get a damage boost depending on the target entity type
 * (via {@link TagKey} with the {@link EntityType} subtype)
 *
...
public class OilEffect {
    ...
    public static final IUsePredicate USE_NOTHING =
            (baseDamage, effect, level, target, user) -> false;
    public static final IUsePredicate USE_UNDEAD =
            (baseDamage, effect, level, target, user) -> target.getType().is(EntityTypeTags.UNDEAD);
    public static final IUsePredicate USE_ARTHROPOD =
            (baseDamage, effect, level, target, user) ->
                    target.getType().is(EntityTypeTags.ARTHROPOD);
    public static final IUsePredicate USE_CRYOTIC =
            (baseDamage, effect, level, target, user) ->
                    target.getType().is(EntityTypeTags.FREEZE_HURTS_EXTRA_TYPES);
    public static final IUsePredicate USE_NECTROTIC =
            (baseDamage, effect, level, target, user) ->
                    target.getType().is(ModEntityTypeTags.HUMANOIDS);
    public static final IUsePredicate USE_CREEPER =
            (baseDamage, effect, level, target, user) ->
                    target.getType().is(ModEntityTypeTags.CREEPERS);
    public static final IUsePredicate USE_AQUATIC =
            (baseDamage, effect, level, target, user) ->
                    target.getType().is(EntityTypeTags.AQUATIC);
    public static final IUsePredicate USE_ENDER =
            (baseDamage, effect, level, target, user) ->
                    target.getType().is(ModEntityTypeTags.ENDER);
    ...
    public float onUse(
        ...
        return this.usePredicate.test(baseDamageIn, this, levelIn, targetEntityIn, userEntityIn)
                ? baseDamageIn + (baseDamageIn * this.getDamageModifier())
                : baseDamageIn;
    }
