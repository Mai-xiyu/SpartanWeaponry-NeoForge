...
public class ModCommands {
    private static final DynamicCommandExceptionType ERROR_NO_ITEM = new DynamicCommandExceptionType((object) ->
            Component.translatable("command." + ModSpartanWeaponry.ID + ".apply_oil.error.no_item", object));
    ...
    private static int applyOil(CommandSourceStack commandSourceIn, ServerPlayer playerIn, OilInput oilIn) throws CommandSyntaxException {
        ...
        IOilHandler oilHandler = ModCapabilities.getOilHandler(applyStack);
        if (oilHandler == null)
            throw ERROR_INCOMPATIBLE_ITEM.create(applyStack.getItem().getName(applyStack).getString());
